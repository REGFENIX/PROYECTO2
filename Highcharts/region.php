...
$stmt = $conn->prepare("SELECT region, count(id_residencia) FROM residencias WHERE region= 'PUBLICO' ");
                $stmt->execute();
                $result = $stmt->get_result();
                if ($result->num_rows === 0) exit('No hay registros en la BD');


$stmt2 = $conn->prepare("SELECT region, count(id_residencia) FROM residencias WHERE region= 'PRIVADO' ");
                $stmt2->execute();
...
<figure class="highcharts-figure">
    ...
    <center><p class="highcharts-description">
       Porcentaje de Residencias por Region
    </p></center>
</figure>
...
while ($row= $result->fetch_array()) {
         echo "['".$row["region"]."',".$row["count(id_residencia)"]."],";

        } ?>
        <?php
while ($row2= $result2->fetch_array()) {
         echo "['".$row2["region"]."',".$row2["count(id_residencia)"]."]";
